...
public class GraphSearch<S, A> extends TreeSearch<S, A> {
	...
	private void cleanUpFrontier() {
		while (!frontier.isEmpty() && frontier.element().getPathCost() > reached.get(frontier.element().getState()).getPathCost()) { // &&																			// reached.get(frontier.element().getState()).getPathCost()
			frontier.remove();
		}
	}
}